classdef DeepAgent < Player
    ...
    properties
        ...
        % for precomputing net outputs
        last_bets_range = [-10,0:20];
        bet2betsub = @(y) (y==-10) + (y~=-10).*(y+2);
        betsub2ind = -ones(length(last_bets_range),...
            length(last_bets_range),length(last_bets_range));
    end
    ...
    methods
        ...
        function [] = debrief(obj,reward,total_heads,hand)
            ...
            if obj.gamesSinceLastTrain >= obj.gamesBetweenTraining
                if(obj.training)
                    ...
                    inputs_to_nn = zeros(length(obj.last_bets_range)^3,3);
                    
                    itr = 1;
                    for i = 2:length(obj.last_bets_range)
                        bet_i = obj.last_bets_range(i);
                        possible_prev_bets_j = obj.last_bets_range(1:i-1);
                        
                        last_bet = [bet_i,-10,-10];
                        inputs_to_nn(itr) = last_bet;
                        obj.betsub2ind( = itr;
                        itr = itr+1;
                        for j = 2:length(possible_prev_bets_j)
                            bet_j = possible_prev_bets_j(j);
                            possible_prev_bets_k = possible_prev_bets_j(1:j-1);
                            
                            last_bet = [bet_i,bet_j,-10];
                            inputs_to_nn(itr) = last_bet;
                            betsub2ind(last_bet) = itr;
                            itr = itr+1;
                            
                            for k = 2:length(possible_prev_bets_k)
                                bet_k = possible_prev_bets_k(k);
                                
                                last_bet = [bet_i,bet_j,bet_k];
                                inputs_to_nn(itr) = last_bet;
                                betsub2ind(last_bet) = itr;
                                itr = itr+1;
                            end
        ...
        function [i] = bet2ind(bet)
            betsub = obj.bet2betsub(bet);
            i = obj.betsub2ind(betsub(1),betsub(2),betsub(3));
        end
    end
